    ...
    switch (pan.state) {
        ...
        default:
            if(location.y > self.keyboardView.frame.origin.y || self.keyboardView.frame.origin.y != self.previousKeyboardY) {
                ...
                newKeyboardY = newKeyboardY > _screenHeight ? _screenHeight : newKeyboardY;
...
- (void)keyboardWillShowHide:(NSNotification *)notification {
    ...
    if (self.keyboardWillChange) {
        ...
        if (![notification.name isEqualToString:UIKeyboardWillShowNotification]){
           keyboardRect = CGRectMake(keyboardRect.origin.x, a, keyboardRect.size.width, keyboardRect.size.height);
