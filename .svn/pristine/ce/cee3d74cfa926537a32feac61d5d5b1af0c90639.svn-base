...
@property(nonatomic,retain) CIImageView *imageView;
...
-(instancetype)initWithFrame:(CGRect)frame
{
    if (self =[super initWithFrame:frame] ) {
        ...
        imageView = [[CIImageView alloc] initWithFrame:CGRectZero];
...
{
    if ([self.imageDelegate respondsToSelector:@selector(SubsribeImageClick:)]) {
        CIImageView *imaegView = (CIImageView *)gesture.view;
        [self.imageDelegate SubsribeImageClick:imaegView.linkUrl];
    }
}
...
-(void)setMessage:(id<XHMessageModel>)message
{
    ...
    [contentLable setFrame:CGRectMake(gap, height+gap, width-gap*2, 15+contentSize.height+gap)];
        ...
        if (imageUrl !=nil && ![imageUrl isKindOfClass:[NSNull class]]) {
                      CGFloat imageHeight =(width-2*gap)*42/65;
...
//                NSMutableArray *tempImageArr =[NSMutableArray arrayWithArray:[defaults objectForKey:@"tempImageUrlSite"]] ;
//                if (tempImageArr == nil) {
