...
//    }
        ...
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (logsErrors) {
                    ...
                    NSLog(@"DB Query: %@", sql);
                    [[CommUtilHelper sharedInstance] setErrorMsg:[[sql stringByAppendingString:@"DB Error"] stringByAppendingString:[self lastErrorMessage]]];
#ifndef NS_BLOCK_ASSERTIONS
                    if (crashOnErrors) {
    ...
    if (idx != queryCount) {
        [[CommUtilHelper sharedInstance] setErrorMsg:[sql stringByAppendingString:@"Error: the bind count is not correct for the # of variables (executeQuery)"]];
