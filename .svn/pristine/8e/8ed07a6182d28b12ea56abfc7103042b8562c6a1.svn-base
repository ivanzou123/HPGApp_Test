...
//    self.viewDidLoad;
    //先隐藏所有的下拉
    ddSource.tv.hidden = YES;
    ddType.tv.hidden = YES;
    ddLevel.tv.hidden = YES;
    ddCity.tv.hidden = YES;
    TFollowType.tv.hidden = YES;
    NSUserDefaults *Parmers = [NSUserDefaults standardUserDefaults];
    NSString *projectName = [Parmers objectForKey:@"CRM_PROJECTNAME"];
    
    [l1  setText:[projectName stringByAppendingString:[NSString stringWithFormat:@"%@",@"-客户新增"]]];
...
- (void)viewDidLoad {
    ...
    //[v1 setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:1]];
     [v1 setBackgroundColor:[UIColor colorWithRed:34/255.0 green:34/255.0 blue:34/255.0 alpha:0.83]];
    //设置uiview名称
    l1 = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2-75,20, 150, 40)];
    NSUserDefaults *Parmers = [NSUserDefaults standardUserDefaults];
    NSString *projectName = [Parmers objectForKey:@"CRM_PROJECTNAME"];
...
//   [self.view addSubview:v4];
    ...
    //主要信息
    vmain = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 92, self.view.frame.size.width, self.view.frame.size.height-120)];
    ...
    switcher.font = [UIFont systemFontOfSize:15];
    ...
    //其他信息
    vother = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 92, self.view.frame.size.width, self.view.frame.size.height-120)];
    ...
    TRemark = [[CBTextView alloc] initWithFrame:CGRectMake(10,185, self.view.frame.size.width-20, 110)];
    TRemark.placeHolder = @"输入客户背景";
    [TRemark.textView.layer setCornerRadius:10];
   // TRemark.textView.delegate = self;
    TRemark.aDelegate= self;
    TRemark.textView.returnKeyType = UIReturnKeyDone;//设置回车键类型
    TRemark.textView.font = [UIFont fontWithName:@"Arial" size:15];
...
-(void)SaveCustInfoThread
{
    ...
    NSMutableDictionary *CustParmers = [NSMutableDictionary dictionaryWithCapacity:15];
     ...
     [CustParmers setObject:TRemark.textView.text forKey:@"Remark"];
     ...
     [parmer setObject:@"saveCustInfo" forKey:@"method"];
...
-(void)startgetSelectThread
{
    ...
    NSString *soapMessage=@"method=getSelectList&parameter=";
    soapMessage = [soapMessage stringByAppendingString:[NSString stringWithFormat:@"[%@]",projectID]];
...
-(void)startgetPreferInThread
{
    ...
    NSString *soapMessage=@"method=getPreferList&parameter=";
    soapMessage = [soapMessage stringByAppendingString:[NSString stringWithFormat:@"[%@]",projectID]];
    ResponseModel *response = [CrmNetServiceUtil startRecommandlistByPage:1 Message:soapMessage];
    isLoadingNext = NO;
