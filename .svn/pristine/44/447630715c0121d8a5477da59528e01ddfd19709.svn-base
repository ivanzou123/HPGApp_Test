 ...
 *发送图片前确认视图
 */
@property (nonatomic,strong) UIView *disImageView;
/**
 *  管理地理位置的工具对象
 */
...
@synthesize firsLoad;
@synthesize allowScrollToBottom;
...
////                UIAlertView *exView = [[UIAlertView alloc] initWithTitle:@"promt" message:[@"error" stringByAppendingString: [exception description] ]delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
////                [exView show];
//                   //[weakSelf.messageTableView insertRowsAtIndexPaths:indexPaths withRowAnimation:UITableViewRowAnimationNone];
//        
//            //}
...
- (void)addMessage:(XHMessage *)addedMessage {
        ...
        [self.messageTableView insertRowsAtIndexPaths:indexPaths withRowAnimation:UITableViewRowAnimationNone];
            ...
            //}
    if([self checkIsTableViewBottom]) {
 ...
 *  @return bool
 */
-(BOOL)checkIsTableViewBottom
{
 
    if (self.messageTableView.contentOffset.y == (self.messageTableView.contentSize.height-self.messageTableView.frame.size.height+self.messageInputView.frame.size.height)) {
        return true;
    }else
    {
        return false;
...
- (void)initilzer {
    ...
    inputView.allowsSendMultiMedia = self.allowsSendMultiMedia;
     inputView.allowLoadMenu = self.allowLoadMenu;
...
-(void)setMenuMutableArr:(NSMutableArray *)arr
{
 [_messageInputView setMenuArr:self.menuArr];
}

- (void)viewWillAppear:(BOOL)animated{
    _messageInputView.allowLoadMenu=self.allowLoadMenu;
    ...
    //[self.messageTableView handleWillHideKeyboardNotification:(NSNotification *)uikeyboard];
    
    //切换语音按钮为文本
    UIButton *b=[[UIButton alloc] initWithFrame:CGRectZero];
    b.tag=3;
    [self.messageInputView messageStyleButtonClicked:b];
    //
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    //[self.messageTableView handleWillHideKeyboardNotification:(NSNotification *)UIKeyboardWillHideNotification];
...
- (void)viewDidLoad {
    [super viewDidLoad];
   // tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideMenuBar)];
...
- (NSString *)getRecorderPath {
   ...
   // dateFormatter.dateFormat = @"yy-MMMM-dd";
    recorderPath =  [[CommUtilHelper sharedInstance] dataVoicePath:[[[CommUtilHelper sharedInstance] createUUID] stringByAppendingString:@"-MySound.caf"] ]; //[[CommUtilHelper sharedInstance] createUUID];
...
#pragma mark predisimageview

-(void)didSendPhotoPreConfirm:(PreDisImageViewController *)con
{
    if ([self.delegate respondsToSelector:@selector(preDisSendImageView:)]) {
        [self.delegate preDisSendImageView:con];
    }
}
...
- (void)didSendMessageWithPrePhoto:(UIImage *)photo {
    DLog(@"send photo : %@", photo);
...
- (void)layoutOtherMenuViewHiden:(BOOL)hide {
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        //CGRect xx =  self.messageInputView.frame;
...
- (void)didFinishRecoingVoiceAction {
    ...
    if (self.voiceRecordHelper.currentTimeInterval <=kVoiceRecorderTotalTime ) {
        [self finishRecorded];
    }
    ...
    void (^PickerMediaBlock)(UIImage *image, NSDictionary *editingInfo) = ^(UIImage *image, NSDictionary *editingInfo) {
        if (image) {
            ...
            if (CFStringCompare ((__bridge CFStringRef) mediaType, kUTTypeMovie, 0) == kCFCompareEqualTo) {
                ...
                PreDisImageViewController *P=[[PreDisImageViewController alloc] initWithImageView:[editingInfo valueForKey:UIImagePickerControllerOriginalImage] SendController:self];
                [self showPreDisController:P];
                //[self targetViewControllerForAction:nil sender:P];
                //[weakSelf.navigationController presentViewController:P animated:YES completion:nil];
                //[self showViewController:P sender:nil];
...
-(void)showPreDisController:(PreDisImageViewController *)con
{
    [self didSendPhotoPreConfirm:con];
}
...
- (void)didSelecteEmotion:(XHEmotion *)emotion atIndexPath:(NSIndexPath *)indexPath {
    if (emotion.emotionPath) {
        NSString *text = [self.messageInputView.inputTextView  getText];
        NSMutableArray *mArr = [[NSMutableArray alloc] initWithArray: [emotion.emotionPath componentsSeparatedByString:@"/"]];
        NSString *fileName = nil;
        if([mArr count]) {
            fileName=[mArr objectAtIndex:[mArr count]-1];
            mArr = [[NSMutableArray alloc] initWithArray: [fileName componentsSeparatedByString:@"."]];
            fileName = [mArr objectAtIndex:0];
        }
        fileName =  [EmotionHelper codeToEmotion:fileName];
        if (fileName) {
            [self.messageInputView.inputTextView setText:[text stringByAppendingString:fileName]];
        }
           ...
           // if (scrollView.contentOffset.y >=0 && scrollView.contentOffset.y <= 44) {
            if ( scrollView.contentOffset.y >=0 && scrollView.contentOffset.y <= 64) {
                if (!self.loadingMoreMessage) {
                    NSLog(@"contentoffset:%f,%f",scrollView.contentOffset.y,scrollView.contentOffset.y);
...
//    }
    ...
    if ([NSStringFromClass([touch.view class]) isEqualToString:@"UITableViewCellContentView"] || [NSStringFromClass([touch.view class]) isEqualToString:@"XHMessageBubbleView"]) {
        [self layoutOtherMenuViewHiden:YES];
         return YES;
    }else
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(shouldDisplayTimestampForRowAtIndexPath:)]) {
     ...
     //NSString *cellIdentifier = [@"XHMessageTableViewCell_" stringByAppendingString:[NSString stringWithFormat:@"%i",(int)indexPath.row]];
    NSString *cellIdentifier = @"XHMessageTableViewCell";
    ...
    if (!messageTableViewCell) {
        ...
        messageTableViewCell.messageBubbleView.displayTextView.delegate=self;
    }
