...
@interface CustomRootRefreshTableViewController ()

@property (nonatomic, strong) XHRefreshControl *customRefreshControl;

@end

@implementation CustomRootRefreshTableViewController
...
- (void)setupRefreshControl {
    if (!_customRefreshControl) {
        ...
        _customRefreshControl.activeIndiactorColor = self.activeLoadIniactorColor;
        //NSLog(@"%@p",self.activeLoadIniactorColor.CGColor);
    }
}
...
- (id)init {
    ...
    if (self) {
        ...
        self.activeLoadIniactorColor=[UIColor grayColor];
...
{
   return [self dateToStr:[NSDate date]];
}
